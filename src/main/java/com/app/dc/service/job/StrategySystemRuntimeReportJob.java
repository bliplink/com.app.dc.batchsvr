...
@Slf4j
public class StrategySystemRuntimeReportJob implements Job {
    ...
    public synchronized void runJob(BatchStart batchStart) {
        if (runFlag) {
            log.warn("StrategySystemRuntimeReportJob is runing.");
            return;
        }
        runFlag = true;
        try {
            batchStart.runStrategySystemRuntimeReportJob();
        } finally {
            runFlag = false;
        }
    }
